...
that answers three questions for every other real project it discovers
via each sibling checkout's own `hydra-umc.project.json` manifest - the
count itself is never hardcoded here, since it grows as the ecosystem
does:
...
Not every discovered project belongs on the CM5 itself, either - most
...
- **`deploy` is a classification, not a restriction.** Treating every
  discovered project as "a thing that belongs on the CM5" was wrong - firmware repos
  ...
  meant to run on a developer's own PC where every discovered project is
  fair game to inspect.
